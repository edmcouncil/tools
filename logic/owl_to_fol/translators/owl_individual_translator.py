...
def translate_rdf_triple_about_individual_subject_to_fol(rdf_triple: tuple, owl_ontology: Graph):
    triple_subject = get_fol_symbol_for_owl_node(node=rdf_triple[0], rdf_graph=owl_ontology)
    triple_predicate = get_fol_symbol_for_owl_node(node=rdf_triple[1], rdf_graph=owl_ontology, arity=2)
    triple_object = get_fol_symbol_for_owl_node(node=rdf_triple[2], rdf_graph=owl_ontology)
    if rdf_triple[1] == RDF.type:
        get_subformula_from_node(node=rdf_triple[2], variables=[triple_subject], rdf_graph=owl_ontology)
        return
