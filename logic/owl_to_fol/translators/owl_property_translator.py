...
def translate_rdf_triple_about_property_to_fol(rdf_triple: tuple, owl_ontology: Graph):
    ...
    if rdf_triple[1] == RDF.type:
        if rdf_triple[2] == OWL.TransitiveProperty:
            property_formula_1 = get_subformula_from_node(node=rdf_triple[0], owl_ontology=owl_ontology, variables=[first_variable, second_variable])
        ...
        if rdf_triple[2] == OWL.SymmetricProperty:
            ...
            QuantifyingFormula(
                ...
                is_self_standing=True)
            return
    antecedent = get_subformula_from_node(node=rdf_triple[0], owl_ontology=owl_ontology, variables=[first_variable, second_variable])
    subsequent = get_subformula_from_node(node=rdf_triple[2], owl_ontology=owl_ontology, variables=[first_variable, second_variable])
    if not antecedent:
    ...
    if rdf_triple[1] == RDFS.range:
        subsequent = get_subformula_from_node(node=rdf_triple[2], owl_ontology=owl_ontology, variables=[second_variable])
        QuantifyingFormula(
            quantified_formula=Implication(arguments=[antecedent, subsequent]),
            variables=[first_variable, second_variable],
            quantifier=Quantifier.UNIVERSAL,
